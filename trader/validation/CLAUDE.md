...
→ CVD filter is 2x more valuable than others
```
...
Backtest CVD: BEARISH → BLOCKED
Live CVD: NEUTRAL → ENTERED
...
**Example Finding**:
```
📊 ROOM-TO-RUN THRESHOLD SENSITIVITY
Current: 1.5% minimum
...
→ Recommend 2.0% threshold (quality over quantity)
```
...
⚠️  MISSED: TSLA LONG @ 09:55:30
   ...
   → BUG: State machine not transitioning correctly
...
Root Cause: Data resolution mismatch
  → Backtest uses 5-sec bars
  → Live uses tick data
  → Recommendation: Align backtester to ticks
...
```
trader/validation/
├── CLAUDE.md                           # This file - validation system guide
...
├── compare_backtest_vs_live.py         # Detect data/logic discrepancies
├── parameter_sensitivity.py            # Test threshold variations
├── generate_weekly_summary.py          # Weekly aggregate analysis
├── validation_utils.py                 # Shared utilities for all scripts
└── reports/                            # Generated validation reports
    ├── validation_summary_20251021.md
    ├── filter_effectiveness_20251021.md
...
- ✅ **100% confidence** in backtest results
- ✅ **Identify bugs** before they affect live trading
- ✅ **Prove strategy correctness** with data

### 2. Filter Optimization
- ✅ **Measure filter value** (which ones save money?)
- ✅ **Optimize thresholds** with real data
- ✅ **Remove ineffective filters** (reduce complexity)

### 3. Live Trading Confidence
- ✅ **Verify live behavior** matches backtest
- ✅ **Detect data quality issues** immediately
- ✅ **Debug filter issues** as they happen

### 4. Strategy Evolution
- ✅ **Data-driven decisions** on filter changes
- ✅ **Quantify improvements** with hard numbers
- ✅ **Avoid overfitting** with sensitivity analysis
...
```
⚠️  MISSED WINNER: SMCI LONG (+12.2%)
...
**Core Workflow**:
```
Scanner Results → Entry Log → Market Data → Validation Report
     ↓               ↓             ↓               ↓
...
```python
if gain_pct >= 2.0:
    outcome = 'WINNER'       # Real opportunity (≥2% move)
elif gain_pct >= 1.0:
...
for breakout in breakouts:
    ...
    if log_entry:
        ...
        if decision == 'ENTERED' and outcome == 'WINNER':
            classification = 'CORRECT_ENTRY'      # ✅ Entered winner
        elif decision == 'BLOCKED' and outcome in ['CHOPPY', 'STOPPED_OUT']:
            classification = 'CORRECT_BLOCK'      # ✅ Blocked loser
        elif decision == 'ENTERED' and outcome in ['CHOPPY', 'STOPPED_OUT']:
            classification = 'BAD_ENTRY'          # ❌ Entered loser
        elif decision == 'BLOCKED' and outcome == 'WINNER':
            classification = 'MISSED_WINNER'      # ❌ Blocked winner
```
...
- Entered winners: ✅ Strategy working correctly
- Blocked winners: ❌ Opportunity missed, investigate
- Entered losers: ❌ Filter failure, fix required
- Blocked losers: ✅ Filters saved money
