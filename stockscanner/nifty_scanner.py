...
Finds high-probability breakout setups for Nifty 50 stocks
"""
...
import argparse
import pytz
from nifty_symbols import get_nifty_symbols, NIFTY_SECTORS

class NiftyScanner:
    """Nifty 50 scanner for PS60 trading setups"""
    ...
    def get_symbols(self, category='all'):
        """
        Get Nifty 50 symbols to scan based on category

        Args:
            category: 'all', 'quick', or sector name ('it', 'banking', 'auto', etc.)
        """
        return get_nifty_symbols(category)
    ...
    def analyze_breakout_levels(self, df, resistance, support, current_price):
        ...
        # Check for psychological levels (Indian Rupees)
        if resistance % 10 == 0:
    ...
    def scan_symbol(self, symbol, historical_date=None):
        ...
        """
        try:
            # Create and qualify contract for Indian stocks
            # NSE = National Stock Exchange of India, INR = Indian Rupee
    ...
    def run_scan(self, symbols=None, category='all', historical_date=None):
        ...
        print("\n" + "="*80)
        if historical_date:
            print(f"NIFTY 50 SCANNER - HISTORICAL ({historical_date.strftime('%Y-%m-%d')})")
        else:
            print("NIFTY 50 BREAKOUT SCANNER - NSE INDIA")
        print("="*80)
    ...
    def save_results(self):
        ...
        csv_filename = f'output/nifty_scanner_results_{trading_date}.csv'
        json_filename = f'output/nifty_scanner_results_{trading_date}.json'
        ...
        df.to_csv('output/nifty_scanner_results.csv', index=False)
        with open('output/nifty_scanner_results.json', 'w') as f:
            json.dump(self.results, f, indent=2, default=str)

        print(f"\n✅ Results saved to:")
        print(f"   - {csv_filename}")
        print(f"   - {json_filename}")
        print(f"   - output/nifty_scanner_results.csv (latest)")
        print(f"   - output/nifty_scanner_results.json (latest)")


def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description='Nifty 50 Breakout Scanner (NSE India)')
    parser.add_argument('--symbols', nargs='+', help='Specific Nifty 50 symbols to scan')
    parser.add_argument('--category', default='all',
                       choices=['all', 'quick', 'it', 'banking', 'auto', 'pharma',
                               'fmcg', 'metals', 'energy', 'finance'],
                       help='Category of Nifty stocks to scan')
    parser.add_argument('--client-id', type=int, default=1002, help='TWS client ID')
